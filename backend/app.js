...
// Function to format report in the HTML format
function formatReportInHTMLformat(comparisonResults, URL) {
  ...
  const HTMLformattedReportArray = comparisonResults.map((currentIssue, index) => {
    return outputString = '<div style="margin: 1%; border-style:solid; border-width: 2px; padding: 4px; border-radius: 10px; background-color: #fdd; border-color: #ff9696;">' +
      '<div style="margin: 5px 0px; font-weight: normal;"> <b>Error ID:</b> ' + index + ' </div>' +
      '<div style="margin: 5px 0px; font-weight: normal;"><b>Error Message:</b> ' + currentIssue['message'] + '</div>' +
      '<div style="margin: 5px 0px; font-weight: normal;"> <b>Error Code:</b> ' + currentIssue['code'] + '</div>' +
      '<div style="margin: 5px 0px; font-weight: normal;"> <b>Error Context:</b> <code>' + htmlEncode(currentIssue['context']) + '</code> ' + ' <b>[(select with ' + currentIssue['selector'] + ')]</b> </div>' +
      '<div style="margin: 5px 0px; font-weight: normal;"> <b>Existed In Previous Report</b>: ' + currentIssue['existedInPreviousReport'] + '</div>' + '</div>';
  });

  const HTMLformattedReport = '<h3 style="text-align:center; font-family: Arial, Helvetica, sans-serif;"> [<u>' + URL + ' </u>] Website Accessibility Report </h3>' +
    '<div style="margin-left:1%; margin-right:1%; width: max-content; background-color: #fdd; border-color: #ff9696; padding: 4px; border-style:solid; border-width: 2px; border-radius: 5px;"><b>Total number of errors:</b> ' + HTMLformattedReportArray.length + ' </div>' +
    HTMLformattedReportArray.join('');
  fs.writeFileSync(previousReportInHTMLformatPath, HTMLformattedReport);
  return HTMLformattedReport;
}

async function runPa11y(input_URL) {
  ...
  try {
    await pa11y(input_URL, {
      standard: accessibilityTestStandard,
      ignore: technicalErrorCodeList_A_standard.concat(technicalErrorCodeList_AA_standard)
    }).then(results => {
      const comparedReport = compareResults(results);
      const HTMLformatedReport = formatReportInHTMLformat(comparedReport, input_URL);
      return HTMLformatedReport;
    });
  } catch (error) {
    const errorString = `<div style="text-align:center; font-size:2rem; margin-top: 12rem">An error occured. <br> Please try again ! </div>`;
    const previousReportInHTMLformatPath = "./Accessibility_Reports/index.html";
    fs.writeFileSync(previousReportInHTMLformatPath, errorString);
  }
}
...
app.post('/accessibility', async (req, res) => {
  ...
  const st = await runPa11y(targetUrl);
  res.sendFile(__dirname + '/Accessibility_Reports/index.html');
});
