...
class Author
{
            ...
            switch ( $this->type )
            {
                case 'user':
                    ...
                    switch ( $key )
                    {
                        ...
                        default          : $meta = \get_user_meta( $this->id, 'molongui_author_'.$key, true );
                    }
	...
	public function get_avatar( $size = 'full', $context = 'screen', $source = null, $default = null )
	{
		$avatar = '';
		$attr   = array();
        $opt    = \molongui_get_plugin_settings( MOLONGUI_AUTHORSHIP_ID, array( 'main', 'box' ) );
        if ( is_array( $size ) )
        {
            $width  = $size[0];
            $height = $size[1];
        ...
        else
        {
            ...
            $size   = in_array( $size, $sizes ) ? $size : 'authorship-box-avatar';
		...
		if ( $context == 'box' )
		{
            if ( \molongui_authorship_is_feature_enabled( 'box_styles' ) )
            {
            ...
            if ( \molongui_authorship_is_feature_enabled( 'microdata' ) ) $attr = \array_merge( $attr, array( 'itemprop' => 'image' ) );
		}
        switch ( !empty( $source ) ? $source : $opt['avatar_src'] )
        {
            case 'gravatar':
            ...
            default:
                ...
                {
                    switch ( $opt['avatar_local_fallback'] )
                    {
                        case 'gravatar':
	...
	{
        if ( empty( $settings ) ) $settings = \molongui_get_plugin_settings( MOLONGUI_AUTHORSHIP_ID, array( 'main', 'box' ) );
        ...
        if ( \molongui_authorship_is_feature_enabled( 'microdata' ) ) $attr['extra_attr'] .= 'itemprop = "image"';
	...
	public function get_acronym ( $name, $settings = array() )
	{
		...
		if ( empty( $settings ) ) $settings = \molongui_get_plugin_settings( MOLONGUI_AUTHORSHIP_ID, array( 'box' ) );
		$html  = '';
    ...
    public function get_posts( $args = null )
    {
        ...
        switch ( $parsed_args['post_type'] )
        {
            ...
            case 'selected':
                $parsed_args['post_type'] = \molongui_supported_post_types( MOLONGUI_AUTHORSHIP_ID, 'all', false );
            break;
        ...
        $hash_f = \md5( \serialize( $parsed_args ) );
        $key_f  = 'posts' . '_' . $hash_f;
        $posts  = \wp_cache_get( $key_f, MOLONGUI_AUTHORSHIP_NAME );
        if ( false === $posts )
        {
            if ( $this->type == 'guest' )
            {
            ...
            else
            {
                ...
                $hash  = \md5( \serialize( $args2 ) );
                $key   = 'posts' . '_' . $hash;
                $data2 = \wp_cache_get( $key, MOLONGUI_AUTHORSHIP_NAME );

                if ( false === $data2 )
                {
                    $data2 = new \WP_Query( $args2 );
                    $data2 = $data2->posts;
                    \wp_cache_set( $key, $data2, MOLONGUI_AUTHORSHIP_NAME );
                    $db_key = MOLONGUI_AUTHORSHIP_DB_PREFIX . 'cache_posts';
                    $hashes = \get_option( $db_key, array() );
                    $update = \update_option( $db_key, !\in_array( $hash, $hashes ) ? \array_merge( $hashes, array( $hash ) ) : $hashes, true );
                }
                $data = \array_merge( ( \is_array( $data1 ) ? $data1 : array() ), ( \is_array( $data2 ) ? $data2 : array() ) );
                ...
                $hash  = \md5( \serialize( $args3 ) );
                $key   = 'posts' . '_' . $hash;
                $posts = \wp_cache_get( $key, MOLONGUI_AUTHORSHIP_NAME );

                if ( false === $posts )
                {
                    $posts = new \WP_Query( $args3 );
                    $posts = $posts->posts;
                    \wp_cache_set( $key, $posts, MOLONGUI_AUTHORSHIP_NAME );
                    $db_key = MOLONGUI_AUTHORSHIP_DB_PREFIX . 'cache_posts';
                    $hashes = \get_option( $db_key, array() );
                    $update = \update_option( $db_key, !\in_array( $hash, $hashes ) ? \array_merge( $hashes, array( $hash ) ) : $hashes, true );
                }
            }
            \wp_cache_set( $key_f, $posts, MOLONGUI_AUTHORSHIP_NAME );
            $db_key = MOLONGUI_AUTHORSHIP_DB_PREFIX . 'cache_posts';
            $hashes = \get_option( $db_key, array() );
            $update = \update_option( $db_key, !\in_array( $hash_f, $hashes ) ? \array_merge( $hashes, array( $hash_f ) ) : $hashes, true );
        }
