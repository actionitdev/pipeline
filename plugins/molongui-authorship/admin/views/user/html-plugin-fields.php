...
<div id="molongui-user-fields">
    ...
    <!-- Profile Bio -->
    <?php include apply_filters( 'authorship/edit/user/bio/tmpl', 'html-profile-bio.php' ); ?>

    <!-- Profile Picture -->
    <?php include 'html-profile-picture.php'; ?>
    ...
    <?php if ( !empty( $this->vars->networks ) ) : ?>
        ...
        <div id="molongui-social-profiles">
            ...
            <div id="m-social">
                ...
                <?php foreach ( $this->vars->networks as $id => $network ) : ?>
                    ...
                    <div class="m-field <?php echo ( ( !MOLONGUI_AUTHORSHIP_IS_PRO and $network['premium'] ) ? 'm-premium' : '' ) ?>">
                        ...
                        <?php if ( !MOLONGUI_AUTHORSHIP_IS_PRO and $network['premium'] ) : ?>
                            <div class="input-wrap"><input type="text" class="text m-tiptip" disabled placeholder="<?php printf( __( '%s  Premium feature', 'molongui-authorship' ), '&#xf002;' ); ?>" data-tip="<?php echo molongui_premium_tip(); ?>" id="molongui_author_<?php echo $id; ?>" name="molongui_author_<?php echo $id; ?>" value=""></div>
                        <?php else : ?>
    ...
    <?php if ( molongui_authorship_is_feature_enabled( 'box' ) ) : ?>
                    ...
                    <tr class="user-m-box-display-wrap">
                        ...
                        <td>
                            ...
                            <select name="molongui_author_box_display" id="molongui_author_box_display">
                                ...
                                <option value="show"    <?php selected( $user_box_display, 'show' ); disabled( MOLONGUI_AUTHORSHIP_IS_PRO, false ); ?>><?php _e( 'Show', 'molongui-authorship' ); ?></option>
                                <option value="hide"    <?php selected( $user_box_display, 'hide' ); disabled( MOLONGUI_AUTHORSHIP_IS_PRO, false ); ?>><?php _e( 'Hide', 'molongui-authorship' ); ?></option>
                            </select>
                            <?php if( !MOLONGUI_AUTHORSHIP_IS_PRO ) : ?>
                                <span class="description">
                                    <span class="premium"><?php _e( 'Premium', 'molongui-authorship' ); ?></span>
                                    <?php printf( __( "Disabled options are available only in the %spremium version%s of the plugin.", 'molongui-authorship' ), '<a href="' . MOLONGUI_AUTHORSHIP_WEB . '" target="_blank">', '</a>' ); ?>
                                </span>
                            <?php endif; ?>
                        </td>
                    </tr>
    ...
    <?php if ( $user->ID != 0 ) : ?>
        ...
        <!-- Author Tools -->
        <?php include apply_filters( 'authorship/edit/user/tools/tmpl', 'html-profile-tools.php' ); ?>
